...
#ifndef BENCHMARK_OCTREE_H
#define BENCHMARK_OCTREE_H
...
{
 public:
  OctreeParams(uint32_t bucketSize = 32, bool copyPoints = false, double minExtent = 0)
...
Octree<PointT, ContainerT>::Octant::Octant()
    : isLeaf(true), x(0), y(0), z(0), extent(0), start(0), end(0), size(0)
{
...
void Octree<PointT, ContainerT>::initialize(const ContainerT& pts, const OctreeParams& params)
{
  ...
  double maxextent = 0.5 * (max[0] - min[0]);
  ctr[0] += maxextent;
